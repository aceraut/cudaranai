...
void conv_backward(
  ...
  CHECK_EQ(
      input_grad_shape.size(), 4, "conv_backward: input gradient shape error");
  ...
  utils::set_array_cache(
      cache,
      "filter_grad_unfolded",
      {batch_size, out_feats, in_feats * filter_h * filter_w});
  ops::matmul(
      cache["filter_grad_unfolded"].get(), output_grad, cache["col_t"].get());
  ...
  filter->reshape({out_feats, in_feats * filter_h * filter_w});
  utils::set_array_cache(
      cache, "filter_t", {in_feats * filter_h * filter_w, out_feats});
