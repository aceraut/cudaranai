...
__global__ void dropout_forward_kernel(int size, float *output,
  ...
  CUDA_GRID_STRIDE_LOOP(idx, size) {
    ...
    mask[idx] = keep;
    output[idx] = keep ? input[idx] / (1.0 - drop_rate + EPS) : 0.0;
  }
}
...
void dropout_forward(Array *output, const Array *input, float drop_rate,
  ...
  dropout_forward_kernel<<<grid_size, BLOCK_SIZE>>>(size, output_raw, input_raw,
                                                    mask_raw, drop_rate, seed);
...
__global__ void dropout_backward_kernel(int size, float *input_grad,
  ...
  CUDA_GRID_STRIDE_LOOP(idx, size) {
    char keep = mask[idx];
    input_grad[idx] = keep ? output_grad[idx] / (1.0 - drop_rate + EPS) : 0.0;
  }
}
